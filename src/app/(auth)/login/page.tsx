...
import { Mail, Lock, Store, Users, Phone, Eye, EyeOff, Loader2 } from "lucide-react";
  ...
  const handleStaffLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (role === "owner") {
        ...
        router.push("/dashboard/staff-lookup");
      } else {
        router.push("/dashboard");
      }
    } catch {
    ...
    <div className="flex min-h-screen items-center justify-center bg-brand-ivory px-4">
      ...
      >
        ...
        <div className="text-center">
          <div className="mx-auto mb-3 flex h-16 w-16 items-center justify-center rounded-2xl bg-brand-wine text-2xl font-bold text-white shadow-lg">
            AT
          </div>
        ...
        <div className="flex rounded-xl border border-gray-200 bg-white p-1">
          <button type="button" onClick={() => { setTab("staff"); setError(""); }}
            className={`flex flex-1 items-center justify-center gap-2 rounded-lg py-2.5 text-sm font-medium transition-colors ${
              tab === "staff" ? "bg-brand-wine text-white shadow-sm" : "text-gray-500 hover:text-gray-700"
            }`}>
            <Store size={15} /> Staff / Owner
          </button>
          <button type="button" onClick={() => { setTab("customer"); setError(""); }}
            className={`flex flex-1 items-center justify-center gap-2 rounded-lg py-2.5 text-sm font-medium transition-colors ${
              tab === "customer" ? "bg-brand-wine text-white shadow-sm" : "text-gray-500 hover:text-gray-700"
            }`}>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-5 space-y-4">
            ...
            <form onSubmit={handleStaffLogin} className="space-y-3">
              <div className="relative">
                ...
                <Input type="email" placeholder="owner@adityatextile.com" className="pl-9"
                  value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} required />
              </div>
              <div className="relative">
                <Lock size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                <Input type={showPass ? "text" : "password"} placeholder="Password" className="pl-9 pr-10"
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-5 space-y-4">
            <div>
              ...
              <div className="mt-2 rounded-xl bg-amber-50 border border-amber-200 p-2 text-xs text-amber-700">
                Demo: any phone + OTP <code className="font-mono font-bold">123456</code>
              </div>
            </div>
            ...
            {!otpSent ? (
              <div className="space-y-3">
                <div className="relative">
                  ...
                  <Input type="tel" placeholder="+91 98765 43210" className="pl-9"
              ...
              <form onSubmit={handleVerifyOtp} className="space-y-3">
                ...
                <Input type="text" inputMode="numeric" placeholder="123456" maxLength={6}
                  ...
                  value={form.otp} onChange={(e) => setForm((f) => ({ ...f, otp: e.target.value }))} autoFocus />
                <Button type="submit" className="w-full gap-2" disabled={loading || form.otp.length < 6}>
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-gray-400"><Loader2 className="animate-spin" /></div>}>
      <LoginForm />
    </Suspense>
  );
}
