...
function LoginForm() {
  ...
  const handleStaffLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fetch session to determine role, then redirect
      const res = await fetch("/api/auth/session");
      const session = await res.json();
      const role = (session?.user as { role?: string } | undefined)?.role;
      if (role === "staff") {
        window.location.href = "/dashboard/staff-lookup";
      } else {
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin text-gray-300" /></div>}>
      <LoginForm />
    </Suspense>
  );
}
