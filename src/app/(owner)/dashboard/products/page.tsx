...
function ExcelImportDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const handleFile = (f: File|null) => { if(!f)return; if(!f.name.match(/\.(xlsx|csv)$/i)){toast({title:"Upload .xlsx or .csv",variant:"error"});return;} setTimeout(()=>setStep("preview"),800); toast({title:`Reading "${f.name}"...`}); };
  return (
    <Dialog open={open} onOpenChange={()=>{onClose();setStep("upload");}}>
      <DialogContent className="max-w-2xl max-h-[88vh] flex flex-col overflow-hidden p-0">
        <div className="px-6 pt-5 pb-3 border-b border-gray-100"><DialogTitle className="flex items-center gap-2"><FileSpreadsheet size={20} className="text-green-600"/> Import from Excel / CSV</DialogTitle><DialogDescription className="text-xs mt-0.5">Bulk-import products. Photos are optional \u2014 add them individually after import.</DialogDescription></div>
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
          ...
          {step==="preview" && (<div className="space-y-4">
            <div className="flex items-center gap-2 rounded-xl bg-amber-50 border border-amber-200 p-3"><AlertTriangle size={15} className="text-amber-600 shrink-0"/><p className="text-xs text-amber-700">Review below. \u274c rows will be skipped. <strong>Photos not required</strong> \u2014 add individually after import.</p></div>
            {[{name:"Banarasi Silk Saree",brand:"Aditya Textile",cat:"Sarees",price:"\u20b914,999",stock:"5",ok:true},{name:"Block Print Kurti",brand:"XYZ Brand",cat:"Kurtis",price:"\u20b9899",stock:"20",ok:true},{name:"",brand:"",cat:"Unknown",price:"\u20b90",stock:"0",ok:false}].map((row,i)=>(<div key={i} className={`flex items-start gap-3 rounded-xl border p-3 ${row.ok?"border-gray-100 bg-gray-50":"border-red-200 bg-red-50"}`}><span className="text-base">{row.ok?"\u2705":"\u274c"}</span><div className="flex-1 min-w-0"><p className={`text-sm font-medium ${row.ok?"text-gray-800":"text-red-600"}`}>{row.name||"Missing product name"}</p><p className="text-xs text-gray-400 mt-0.5">{row.brand} \u00b7 {row.cat} \u00b7 {row.price} \u00b7 Stock: {row.stock}</p>{!row.ok&&<p className="text-xs text-red-500 mt-1">\u26a0 Name required \u2014 will be skipped</p>}</div><div className="text-right shrink-0"><span className="text-xs text-gray-400">No photo</span><p className="text-[10px] text-gray-300">Add later</p></div></div>))}
            <div className="flex gap-3"><Button variant="outline" className="flex-1" onClick={()=>setStep("upload")}>\u2190 Back</Button><Button className="flex-1 bg-green-600 hover:bg-green-700 gap-2" onClick={()=>{setStep("done");toast({title:"2 products imported!",variant:"success"})};}><CheckCircle size={15}/> Import 2 Valid</Button></div>
          </div>)}
          {step==="done" && (<div className="flex flex-col items-center py-10 gap-4"><div className="flex h-16 w-16 items-center justify-center rounded-full bg-green-100"><CheckCircle size={32} className="text-green-600"/></div><div className="text-center"><p className="font-display text-xl font-bold text-gray-900">Import Complete!</p><p className="text-sm text-gray-500 mt-1">2 imported \u00b7 1 skipped (missing name)</p></div><div className="rounded-xl bg-blue-50 border border-blue-200 p-3 text-xs text-blue-700 text-center max-w-xs">\ud83d\udca1 Products without photos are fully imported. Open any product \u2192 click \u201cImages\u201d tab to add photos.</div><Button onClick={()=>{onClose();setStep("upload");}} className="gap-2 mt-2">Done</Button></div>)}
        </div>
      </DialogContent>
    </Dialog>
  );
