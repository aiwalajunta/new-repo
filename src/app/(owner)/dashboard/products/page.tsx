  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div className="space-y-1"><label className="text-xs font-medium text-brand-text-muted">Category *</label><select {...field("categoryId")} required className="flex h-11 w-full rounded-lg border border-brand-border bg-white px-4 py-2 text-sm text-brand-text focus:outline-none focus:ring-2 focus:ring-brand-wine"><option value="">Select category</option>{CATEGORIES.map((c) => (<option key={c.slug} value={`cat_${c.slug}`}>{c.name}</option>))}</select></div>
        <div className="space-y-1"><label className="text-xs font-medium text-brand-text-muted">Fabric *</label><Input {...field("fabric")} placeholder="Silk, Cotton..." required /></div>
...
export default function OwnerProductsPage() {
  ...
  return (
    <div className="space-y-5 px-4 py-4 pb-24">
      <div className="flex items-center justify-between"><div><h1 className="font-display text-2xl font-bold text-brand-text">Products</h1><p className="text-sm text-brand-text-muted">{products.length} listings</p></div><Button onClick={() => { setEditing(null); setDialogOpen(true); }} className="gap-2" size="sm"><Plus size={16} /> Add</Button></div>
      ...
      {isLoading ? (<div className="space-y-3">{[1,2,3,4].map((i) => <Skeleton key={i} className="h-20 rounded-xl" />)}</div>) : filtered.length === 0 ? (<EmptyState icon={Package} title="No products" description="Add your first product to get started" />) : (
        <div className="space-y-2">{filtered.map((product) => (
          <motion.div key={product.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-3 rounded-xl border border-brand-border bg-white p-3 shadow-sm">
            ...
            <div className="flex-1 min-w-0"><p className="truncate text-sm font-medium text-brand-text">{product.title}</p><div className="mt-0.5 flex items-center gap-2"><span className="text-xs font-bold text-brand-wine">{formatPrice(product.finalPrice)}</span>{product.isNewArrival && <Badge variant="secondary" className="text-[10px] px-1.5 py-0">NEW</Badge>}{!product.isActive && <Badge variant="danger" className="text-[10px] px-1.5 py-0">Inactive</Badge>}</div></div>
            <div className="flex shrink-0 gap-1"><button onClick={() => { setEditing(product); setDialogOpen(true); }} className="flex h-8 w-8 items-center justify-center rounded-lg text-brand-text-muted hover:bg-brand-cream tap-target"><Edit2 size={15} /></button><button onClick={() => deleteMutation.mutate(product.id)} className="flex h-8 w-8 items-center justify-center rounded-lg text-brand-text-muted hover:bg-red-50 hover:text-red-500 tap-target"><Trash2 size={15} /></button></div>
          </motion.div>
        ))}</div>
      )}
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>{editing ? "Edit Product" : "Add New Product"}</DialogTitle><DialogDescription>{editing ? "Update the product details below." : "Fill in the details to add a new product."}</DialogDescription></DialogHeader>
          <ProductForm initial={editing ?? undefined} onSubmit={(data) => createMutation.mutate(data)} isPending={createMutation.isPending} /></DialogContent>
      </Dialog>
    </div>
  );
}
