...
import {
  Plus, Search, Package, Edit2, Trash2, X, Upload,
  FileSpreadsheet, CheckCircle, AlertTriangle, Sparkles,
...
const STORAGE_KEY = "aditya_textile_products_v1";
...
function saveProducts(products: Product[]) {
  ...
  catch { console.error("Failed to save products"); }
}

const empty = (): Partial<Product> => ({
  name: "", sku: "", barcode: "", categoryId: "", brand: "Aditya Textile",
  fabric: "", colors: [], pattern: "", occasions: [], sizes: [],
  purchasePrice: 0, sellingPrice: 0, discountPct: 0, finalPrice: 0,
  stockTotal: 0, stockReserved: 0, stockAvailable: 0,
  imageUrls: [], description: "", notes: "", rackLocation: "",
...
function ProductFormDialog({ product, open, onClose, onSave }: {
  product: Partial<Product> | null; open: boolean; onClose: () => void; onSave: (p: Partial<Product>) => void;
}) {
  ...
  const csvField = (k: keyof Product) => ({
    value: Array.isArray(form[k]) ? (form[k] as string[]).join(", ") : "",
  ...
  const handleImageUpload = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      toast({ title: `${imgs.length} photo(s) uploaded \u2713`, variant: "success" });
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[92vh] overflow-hidden flex flex-col gap-0 p-0">
        <div className="px-6 pt-5 pb-3 border-b border-gray-100">
          ...
          <DialogDescription className="text-xs mt-0.5">{isEdit ? "Changes save permanently to this device." : "Fill details to add to catalog."}</DialogDescription>
        </div>
        <div className="flex border-b border-gray-200 px-6 gap-1">
          {(["details", "images", "stock"] as Tab[]).map((t) => (
            <button key={t} onClick={() => setTab(t)} className={`pb-2.5 px-1 text-sm font-medium border-b-2 transition-colors mr-4 ${tab === t ? "border-brand-wine text-brand-wine" : "border-transparent text-gray-500 hover:text-gray-700"}`}>
              {t === "details" ? "\ud83d\udccb Details" : t === "images" ? "\ud83d\uddbc\ufe0f Photos" : "\ud83d\udce6 Stock"}
            </button>
          ))}
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {tab === "details" && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1"><label className="text-xs font-semibold text-gray-600">SKU / Product Code</label><Input placeholder="SAR-001" value={form.sku ?? ""} onChange={(e) => set("sku", e.target.value)} /></div>
              <div className="space-y-1"><label className="text-xs font-semibold text-gray-600">Category *</label>
                <select value={form.categoryId ?? ""} onChange={(e) => set("categoryId", e.target.value)} className="flex h-11 w-full rounded-lg border border-gray-200 bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-wine">
                  <option value="">Select...</option>{MOCK_CATEGORIES.map((c) => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}
                </select>
              </div>
              <div className="space-y-1"><label className="text-xs font-semibold text-gray-600">Fabric</label>
                <select value={form.fabric ?? ""} onChange={(e) => set("fabric", e.target.value)} className="flex h-11 w-full rounded-lg border border-gray-200 bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-wine">
                  <option value="">Select...</option>{PRODUCT_FABRICS.map((f) => <option key={f} value={f}>{f}</option>)}
                </select>
              </div>
              ...
              <div className="col-span-2 space-y-1"><label className="text-xs font-semibold text-gray-600">Remarks</label><textarea rows={2} placeholder="Available for alteration..." value={form.notes ?? ""} onChange={(e) => set("notes", e.target.value)} className="flex w-full resize-none rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-wine" /></div>
              <div className="col-span-2 space-y-1"><label className="text-xs font-semibold text-gray-600">Occasions</label>
                <div className="flex flex-wrap gap-1.5">{PRODUCT_OCCASIONS.map((occ) => { const on = (form.occasions ?? []).includes(occ); return (<button key={occ} type="button" onClick={() => set("occasions", on ? (form.occasions ?? []).filter((o) => o !== occ) : [...(form.occasions ?? []), occ])} className={`rounded-full px-3 py-1 text-xs font-medium border transition-colors ${on ? "bg-brand-wine text-white border-brand-wine" : "border-gray-200 text-gray-600 hover:border-brand-wine"}`}>{occ}</button>); })}</div>
              </div>
              <div className="col-span-2 flex items-center gap-6 pt-1">
                {([["isActive", "Active listing"], ["isFeatured", "Featured on home"]] as const).map(([k, label]) => (<label key={k} className="flex cursor-pointer items-center gap-2"><input type="checkbox" checked={!!form[k]} onChange={(e) => set(k, e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-brand-wine focus:ring-brand-wine" /><span className="text-sm text-gray-700">{label}</span></label>))}
              </div>
            </div>
          )}
          {tab === "images" && (
            <div className="space-y-4">
              <div onDrop={(e) => { e.preventDefault(); setDragOver(false); handleImageUpload(Array.from(e.dataTransfer.files)); }} onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onClick={() => fileRef.current?.click()}
                className={`flex flex-col items-center gap-3 rounded-2xl border-2 border-dashed p-8 cursor-pointer transition-colors ${dragOver ? "border-brand-wine bg-brand-rose" : "border-gray-200 hover:border-brand-wine hover:bg-gray-50"}`}>
                {uploading ? <><Sparkles size={20} className="animate-spin text-brand-wine" /><span className="text-sm text-brand-wine font-medium">Processing photos...</span></>
                  : <><Upload size={32} className="text-gray-400" /><div className="text-center"><p className="text-sm font-semibold text-gray-700">Tap to upload product photos</p><p className="text-xs text-gray-400 mt-0.5">JPG, PNG, WebP \u00b7 Saved to this device</p></div></>}
              </div>
              ...
              {(form.imageUrls ?? []).length === 0
                ? (<div className="flex flex-col items-center gap-2 py-6 text-center"><ImageIcon size={32} className="text-gray-200" /><p className="text-sm text-gray-400">No photos yet. Tap above to upload.</p></div>)
...
function ExcelImportDialog({ open, onClose, onImport }: { open: boolean; onClose: () => void; onImport: (p: Partial<Product>[]) => void; }) {
  ...
  const fileRef = useRef<HTMLInputElement>(null);
  const COLS = ["Brand", "Category *", "Product Name *", "SKU", "Selling Price *", "Stock *", "Colors", "Sizes", "Fabric", "Rack", "Remarks"];
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      const errs: string[] = [];
      rows.forEach((row, i) => {
        const get = (keys: string[]) => { for (const k of keys) { const v = row[k] ?? row[k.toLowerCase()]; if (v !== undefined && v !== "") return String(v).trim(); } return ""; };
        const name = get(["Product Name", "Name", "PRODUCT NAME", "product name"]);
        const price = Number(get(["Selling Price", "Price", "MRP", "selling price"])) || 0;
        const stock = Number(get(["Stock", "Qty", "Quantity", "STOCK"])) || 0;
        if (!name) { errs.push(`Row ${i + 2}: Missing product name \u2014 skipped`); return; }
        if (price <= 0) { errs.push(`Row ${i + 2}: "${name}" \u2014 invalid price, skipped`); return; }
        const catName = get(["Category", "CATEGORY"]).toLowerCase();
        const cat = MOCK_CATEGORIES.find((c) => c.name.toLowerCase().includes(catName) || catName.includes(c.name.toLowerCase()));
        products.push({ id: `imp_${Date.now()}_${i}`, name, brand: get(["Brand", "Company"]) || "Aditya Textile", sku: get(["SKU", "Code", "Product Code"]), categoryId: cat?.id ?? "", fabric: get(["Fabric", "Material"]), colors: get(["Colors", "Color"]).split(",").map((s) => s.trim()).filter(Boolean), sizes: get(["Sizes", "Size"]).split(",").map((s) => s.trim()).filter(Boolean), rackLocation: get(["Rack", "Rack Location", "Shelf"]), notes: get(["Remarks", "Notes"]), sellingPrice: price, finalPrice: price, discountPct: 0, purchasePrice: 0, stockTotal: stock, stockAvailable: stock, stockReserved: 0, imageUrls: [], occasions: [], pattern: "", barcode: "", tags: [], description: "", isActive: true, isFeatured: false, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
      });
  ...
  const handleImport = () => { onImport(parsed); setStep("done"); toast({ title: `${parsed.length} products imported!`, variant: "success" }); };
    ...
    <Dialog open={open} onOpenChange={reset}>
      <DialogContent className="max-w-2xl max-h-[88vh] flex flex-col overflow-hidden p-0">
        <div className="px-6 pt-5 pb-3 border-b border-gray-100">
          ...
          <DialogDescription className="text-xs mt-0.5">Upload .xlsx or .csv \u00b7 Photos optional \u2014 add after import</DialogDescription>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
          {step === "upload" && (<>
            <div onDrop={(e) => { e.preventDefault(); setDragOver(false); if (e.dataTransfer.files[0]) processFile(e.dataTransfer.files[0]); }} onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onClick={() => fileRef.current?.click()}
              className={`flex flex-col items-center gap-3 rounded-2xl border-2 border-dashed p-10 cursor-pointer transition-colors ${dragOver ? "border-green-500 bg-green-50" : "border-gray-200 hover:border-green-400 hover:bg-gray-50"}`}>
              {loading ? <><FileSpreadsheet size={36} className="text-green-500 animate-pulse" /><p className="text-sm text-gray-600">Reading file...</p></>
                : <><FileSpreadsheet size={40} className="text-green-500" /><div className="text-center"><p className="font-semibold text-gray-700">Drop .xlsx or .csv here</p><p className="text-xs text-gray-400 mt-1">or click to browse</p></div></>}
            </div>
            ...
            <div className="rounded-xl border border-green-200 bg-green-50 p-4 space-y-3">
              <p className="text-sm font-semibold text-green-800">\ud83d\udce5 Template Format</p>
              <p className="text-xs text-green-700">Columns marked * required. Photos NOT needed \u2014 add individually after import.</p>
          ...
          {step === "preview" && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 rounded-xl bg-blue-50 border border-blue-200 p-3"><CheckCircle size={15} className="text-blue-600 shrink-0" /><p className="text-xs text-blue-700"><strong>{parsed.length} products</strong> ready{errors.length > 0 && `, ${errors.length} rows skipped`}.</p></div>
          ...
          {step === "done" && (
            <div className="flex flex-col items-center py-10 gap-4">
              ...
              <div className="text-center"><p className="font-display text-xl font-bold text-gray-900">Import Complete!</p><p className="text-sm text-gray-500 mt-1">{parsed.length} products added</p></div>
              <div className="rounded-xl bg-blue-50 border border-blue-200 p-3 text-xs text-blue-700 text-center max-w-xs">\ud83d\udca1 Open any product \u2192 Photos tab to add images.</div>
...
function DeleteConfirm({ product, onConfirm, onCancel }: { product: Product; onConfirm: () => void; onCancel: () => void }) {
  return (<Dialog open onOpenChange={onCancel}><DialogContent className="max-w-sm"><DialogHeader><DialogTitle>Delete &ldquo;{product.name}&rdquo;?</DialogTitle><DialogDescription>Removes from catalog permanently.</DialogDescription></DialogHeader><div className="flex gap-3 pt-2"><Button variant="outline" className="flex-1" onClick={onCancel}>Cancel</Button><Button className="flex-1 bg-red-600 hover:bg-red-700" onClick={onConfirm}>Yes, Delete</Button></div></DialogContent></Dialog>);
}
...
export default function ProductsPage() {
  ...
  useEffect(() => { setProducts(loadProducts()); setHydrated(true); }, []);

  const updateProducts = useCallback((updater: (prev: Product[]) => Product[]) => {
    setProducts((prev) => { const next = updater(prev); saveProducts(next); return next; });
  }, []);
  ...
  const handleSave = (saved: Partial<Product>) => {
    updateProducts((prev) => saved.id
      ? prev.map((p) => p.id === saved.id ? { ...p, ...saved, updatedAt: new Date().toISOString() } as Product : p)
      : [{ ...saved, id: `prod_${Date.now()}`, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() } as Product, ...prev]
    );
    toast({ title: saved.id ? "Product saved \u2713" : "Product added \u2713", variant: "success" });
  };

  const handleDelete = () => {
    if (!deleteTarget) return;
    updateProducts((prev) => prev.filter((p) => p.id !== deleteTarget.id));
  ...
  const handleImport = (imported: Partial<Product>[]) => {
    updateProducts((prev) => [
      ...imported.map((p) => ({ ...p, id: p.id ?? `imp_${Date.now()}_${Math.random()}`, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }) as Product),
      ...prev,
    ]);
  };
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          {!isOwner && <p className="text-xs text-blue-600 mt-0.5">\ud83d\udc41\ufe0f Read-only \u2014 contact owner to edit</p>}
        </div>
        {isOwner && (
        ...
        : (<AnimatePresence mode="popLayout"><div className="space-y-2">{filtered.map((p, i) => (
            <motion.div key={p.id} layout initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ delay: i * 0.01 }}>
              <Card className="hover:shadow-sm transition-shadow"><CardContent className="flex items-center gap-3 p-3">
                <div className="flex h-14 w-12 shrink-0 items-center justify-center rounded-xl bg-brand-cream text-2xl border border-brand-border overflow-hidden">
                  {(p.imageUrls ?? []).length > 0 ? <img src={p.imageUrls[0]} alt={p.name} className="h-full w-full object-cover" /> : <span>{getCatIcon(p.categoryId)}</span>}
                </div>
                ...
                <div className="flex gap-0.5 shrink-0">
                  {isOwner ? (<><button onClick={() => { setFormProduct({ ...p }); setFormOpen(true); }} title="Edit" className="flex h-9 w-9 items-center justify-center rounded-lg text-gray-400 hover:bg-brand-wine/10 hover:text-brand-wine transition-colors"><Edit2 size={15} /></button><button onClick={() => setDeleteTarget(p)} title="Delete" className="flex h-9 w-9 items-center justify-center rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-500 transition-colors"><Trash2 size={15} /></button></>) : (<button title="View only" className="flex h-9 w-9 items-center justify-center rounded-lg text-gray-300 cursor-default"><Eye size={15} /></button>)}
                </div>
              </CardContent></Card>
            </motion.div>
          ))}</div></AnimatePresence>)
