...
export default function NewAppointmentPage() {
  ...
  const searchResults = productSearch.length > 1
    ? MOCK_PRODUCTS.filter((p) => p.name.toLowerCase().includes(productSearch.toLowerCase()) || p.sku.toLowerCase().includes(productSearch.toLowerCase())).slice(0, 5)
    : [];

  const addProduct = (product: Product) => {
    if (!selectedProducts.find((s) => s.product.id === product.id)) setSelectedProducts((prev) => [...prev, { product, qty: 1, notes: "" }]);
    setProductSearch(""); setShowSearch(false);
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.customerName || !form.customerPhone || !form.date || !form.time) { toast({ title: "Please fill in all required fields", variant: "error" }); return; }
    toast({ title: `Appointment created for ${form.customerName} on ${form.date} at ${form.time}`, variant: "success" });
  };
  ...
  return (
    <div className="space-y-5 max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">Products to Prepare</CardTitle>
          ...
          <CardContent className="pt-0 space-y-3">
            {showSearch && (
              <div className="relative">
                ...
                <Input placeholder="Search by name or SKU..." className="pl-9" value={productSearch} onChange={(e) => setProductSearch(e.target.value)} autoFocus />
            ...
            {selectedProducts.length === 0 ? (
              <p className="py-4 text-center text-sm text-gray-400">Add products the customer wants to view in-store</p>
        ...
        <Button type="submit" className="w-full" size="lg">Create Appointment</Button>
      </form>
    </div>
  );
}
