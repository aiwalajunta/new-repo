    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-sm max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-xl border border-brand-border bg-brand-wine/5 p-3 text-center">
            <p className="text-xs text-gray-500">Selling Price</p>
            <p className="font-display text-xl font-bold text-brand-wine mt-0.5">{formatPrice(product.finalPrice)}</p>
            {product.discountPct > 0 && <p className="text-xs text-gray-400 line-through">{formatPrice(product.sellingPrice)}</p>}
          </div>
          <div className={`rounded-xl border p-3 text-center ${product.stockAvailable === 0 ? "border-red-200 bg-red-50" : product.stockAvailable <= STOCK_LOW_THRESHOLD ? "border-amber-200 bg-amber-50" : "border-green-200 bg-green-50"}`}>
            <p className="text-xs text-gray-500">Available Stock</p>
            <p className={`font-display text-xl font-bold mt-0.5 ${product.stockAvailable === 0 ? "text-red-600" : product.stockAvailable <= STOCK_LOW_THRESHOLD ? "text-amber-600" : "text-green-600"}`}>{product.stockAvailable === 0 ? "OUT" : product.stockAvailable}</p>
            <p className="text-xs text-gray-400">{product.stockAvailable === 0 ? "Not available" : "units"}</p>
          </div>
        </div>
        <div className="space-y-2">{[
          { label: "Brand", value: product.brand },
          { label: "Code / SKU", value: product.sku },
          { label: "Colors", value: product.colors.join(", ") },
          { label: "Sizes", value: product.sizes.join(", ") },
          { label: "Fabric", value: product.fabric },
          { label: "Rack Location", value: product.rackLocation, highlight: true },
          { label: "Occasions", value: product.occasions.join(", ") },
          { label: "Remarks", value: product.notes },
        ].filter((d) => d.value).map((d) => (
          <div key={d.label} className="flex items-start gap-3">
            <span className="text-xs text-gray-400 w-28 shrink-0 pt-0.5">{d.label}</span>
            <span className={`text-sm font-medium flex-1 ${d.highlight ? "text-blue-600" : "text-gray-800"}`}>{d.value}</span>
          </div>
        ))}</div>
      </DialogContent>
    </Dialog>
      ...
      <DialogContent className="max-w-sm">
        ...
        {phase === "idle" && (
          <div className="space-y-4">
            <div onClick={() => fileRef.current?.click()} className="flex flex-col items-center gap-3 rounded-2xl border-2 border-dashed border-gray-200 hover:border-brand-wine hover:bg-gray-50 p-8 cursor-pointer transition-colors">
              <Camera size={36} className="text-gray-400" />
              <div className="text-center"><p className="text-sm font-medium text-gray-700">Take a photo or upload image</p><p className="text-xs text-gray-400 mt-0.5">AI will identify the product</p></div>
            </div>
            <input ref={fileRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={() => runAI()} />
            <div className="rounded-xl bg-brand-cream border border-brand-border p-3 text-xs text-brand-text-muted"><p className="font-semibold text-brand-text mb-1">\ud83e\udd16 How it works</p><p>Point camera at any saree or garment. AI detects fabric, color and pattern, then shows matching products with prices and stock.</p></div>
          </div>
        )}
        {phase === "scanning" && (
          <div className="flex flex-col items-center gap-4 py-8">
            <div className="relative flex h-16 w-16 items-center justify-center"><div className="absolute inset-0 rounded-full border-4 border-brand-wine border-t-transparent animate-spin" /><Sparkles size={24} className="text-brand-gold" /></div>
            <div className="text-center"><p className="font-medium text-gray-800">Analyzing image...</p><p className="text-xs text-gray-500 mt-1">Detecting fabric \u00b7 color \u00b7 pattern</p></div>
          </div>
        )}
        {phase === "done" && (
          <div className="space-y-4">
            <div className="rounded-xl bg-green-50 border border-green-200 p-4 space-y-2">
              ...
              {[["Detected","Saree"],["Fabric","Silk (94%)"],["Colors","Red, Gold"],["Pattern","Zari / Border"]].map(([k,v]) => (
                <div key={k} className="flex items-center gap-2 text-xs"><span className="text-gray-500 w-20">{k}:</span><span className="font-medium text-gray-800">{v}</span></div>
              ))}
            </div>
            <Button className="w-full gap-2" onClick={() => { onResult("silk saree"); onClose(); setPhase("idle"); }}><Search size={15} /> Show Matching Products</Button>
          </div>
        )}
      </DialogContent>
    </Dialog>
            ...
            <button className="w-full text-left" onClick={() => setSelected(p)}>
              <Card className="hover:shadow-md hover:border-brand-wine/30 transition-all cursor-pointer"><CardContent className="flex items-center gap-3 p-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5 flex-wrap text-[11px] text-gray-400">{p.brand && <><span className="font-medium text-gray-600">{p.brand}</span><span>\u00b7</span></>}<span>{getCatName(p.categoryId)}</span>{p.colors.length > 0 && <><span>\u00b7</span><span>{p.colors.slice(0, 2).join(", ")}</span></>}{p.sku && <><span>\u00b7</span><span className="font-mono">{p.sku}</span></>}{p.rackLocation && <><span>\u00b7</span><span className="font-semibold text-blue-600">\ud83d\udccd {p.rackLocation}</span></>}</div>
                </div>
