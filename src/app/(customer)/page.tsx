...
export default function CustomerHomePage() {
  return (
    <div className="space-y-0">
      ...
      <section className="bg-brand-cream px-6 py-4 border-b border-brand-border">
        <div className="flex items-center justify-around gap-4 text-xs text-brand-text-muted">
          <div className="flex items-center gap-1.5"><Clock size={12} /> Mon–Sat, 10am–8pm</div>
          <div className="flex items-center gap-1.5"><Phone size={12} /> +91 98765 43210</div>
          <div className="flex items-center gap-1.5"><MapPin size={12} /> Main Market, City</div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-4 gap-3">
          {MOCK_CATEGORIES.slice(0, 8).map((cat, i) => (
            <motion.div key={cat.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
              <Link href={`/browse?cat=${cat.slug}`} className="flex flex-col items-center gap-2 rounded-2xl border border-brand-border bg-white p-3 shadow-sm hover:border-brand-gold hover:shadow-md transition-all">
      ...
      <section className="mx-4 mb-6 rounded-2xl bg-gradient-to-br from-brand-wine/5 to-brand-gold/5 border border-brand-border p-5">
        ...
        <div className="space-y-3">
          {[{step:"1",title:"Browse Our Collection",desc:"Explore sarees, lehengas, kurtis and more"},{step:"2",title:"Select Products",desc:"Add items you want to view in our showroom"},{step:"3",title:"Book a Showroom Visit",desc:"Choose a date and time that works for you"},{step:"4",title:"We Prepare Everything",desc:"Products will be ready when you arrive"}].map((s) => (
