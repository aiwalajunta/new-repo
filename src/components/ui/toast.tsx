...
const Toast = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Root>, React.ComponentPropsWithoutRef<typeof ToastPrimitives.Root> & { variant?: "default" | "success" | "error" }>(({ className, variant = "default", ...props }, ref) => (
  <ToastPrimitives.Root ref={ref} className={cn("flex w-full items-center gap-3 rounded-xl border p-4 shadow-lg transition-all data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-80 data-[state=open]:slide-in-from-bottom-full", variant === "success" && "border-green-200 bg-green-50 text-green-800", variant === "error" && "border-red-200 bg-red-50 text-red-800", variant === "default" && "border-brand-border bg-white text-brand-text", className)} {...props} />
));
